...
void updateOverlay(int w, int h)
{
	OMAPLFB_DEVINFO *psDevInfo = GetAnchorPtr();
	/* Copied for the initDev function*/
	/* All part related to framebuffer parameter*/
	...
	psPVRFBInfo->ulWidth = w;
	psPVRFBInfo->ulHeight = h;
	...
	IMG_HANDLE hSwapChain)
{
	...
	if (psDevInfo->bDeviceSuspended != OMAP_TRUE)
		OMAPLFBDisableDisplayRegisterAccess();
...
OMAP_BOOL OMAPLFBVSyncIHandler(OMAPLFB_SWAPCHAIN *psSwapChain)
{
	...
	if(!psSwapChain || !psSwapChain->psPVRJTable || !psSwapChain->psVSyncFlips) {
		return bStatus;
	}
	...
	while(psFlipItem && psFlipItem->bValid)
	{	
		if(psFlipItem->bFlipped)
		{
			if(!psFlipItem->bCmdCompleted)
			{
				if(!psSwapChain->psPVRJTable || !psSwapChain->psPVRJTable->pfnPVRSRVCmdComplete) {
					break;
				}
				psSwapChain->psPVRJTable->pfnPVRSRVCmdComplete((IMG_HANDLE)psFlipItem->hCmdComplete, IMG_TRUE);
