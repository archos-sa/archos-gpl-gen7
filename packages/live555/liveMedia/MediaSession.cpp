...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (sdpLine != NULL) {
    ...
    while (1) {
      ...
      if (subsession->parseSDPAttribute_bitrate(sdpLine)) continue;
#ifdef SUPPORT_REAL_RTSP
...
MediaSubsession::MediaSubsession(MediaSession& parent)
  : sessionId(NULL), serverPortNum(0), sink(NULL), miscPtr(NULL),
    ...
    fPlayEndTime(0.0),
    fVideoWidth(0), fVideoHeight(0), fVideoFPS(0), fBitrate(0), fNumChannels(1), fScale(1.0f),
...
Boolean MediaSubsession::parseSDPAttribute_framerate(char const* sdpLine) {
  // Check for a AvgBitRate:integer;41000
  Boolean parseSuccess = False;

  int bitrate;
  int rate;
  if (sscanf(sdpLine, "a=AvgBitRate:integer;%d", &bitrate) == 1) {
    parseSuccess = True;
    fBitrate = (unsigned)bitrate;
  }

  return parseSuccess;
}

Boolean MediaSubsession::parseSDPAttribute_bitrate(char const* sdpLine) {
